...
function checkLogin() {   //Kullanıcının giriş yapıp yapmadığını ve aktifliğini kontrol eder.
...
function yetkiKontrol($izin_verilenler) {   //   Rol yetkisini kontrol eder. Çoklu rol desteği vardır. (1. kodun en iyi yönü)
...
function redirectIfLoggedIn() { // Zaten giriş yapmış kullanıcıyı login sayfasından uzaklaştırır.
    if (isset($_SESSION['personel_id']) && (isset($_SESSION['is_active']) && $_SESSION['is_active'] == 1)) {
...
function yetkisizErisimYonlendir() {  // Yetkisiz erişim durumunda 403 hatası verir ve yönlendirir.
